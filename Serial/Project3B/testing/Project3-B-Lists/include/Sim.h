...
private:
  ...
  Vector3F fSimBoxHalf;               // half of the lengths of each side of
                                      // simulation box
  ParticleList *fParticles;           // List of particles
...
public:
  Sim() : fParticles{new ParticleList()}, fParticleHead{new Particle()} {}
  ...
  Vector3F GetPos(int i) const {
    int j = 0;
    ParticleNode *entry;
    Vector3F result;
    for (entry = fParticles->GetFirst(); entry; entry = entry->GetNext()) {
      if (j == i) {
        result = entry->GetParticle()->r;
        break;
      }
      j++;
    }
